...
mainResult: "EdgeConv GNN: AUC 0.779 na classificação quark/gluon. SimCLR sem supervisão: AUC 0.632 via linear probe. Treinado em CPU — esperado convergir em GPU."
date: "2025-03"
...
O trabalho demonstra um pipeline completo para representar eventos de jatos de partículas como grafos e aplicar GNNs para classificação e aprendizado de representações — a base técnica para o framework de detecção de anomalias proposto.
...
**Construção do grafo:** pixels ativos = nós; features de nó: intensidades dos 3 canais + x_norm, y_norm, r_norm (6 features); arestas via **k-NN k=8** no espaço (η, φ), seguindo o ParticleNet; features de aresta: dx, dy, distância euclidiana, diferença de intensidade.
...
**Fase 1 — Pré-treino não supervisionado (SimCLR):** augmentations via node feature dropout (p=0.1) + edge dropout (p=0.1); GNNEncoder com 2 camadas EdgeConv + dual pooling (mean + max); ProjectionHead MLP 2 camadas com BatchNorm; NT-Xent loss com τ=0.5; subconjunto estratificado de 20.000 eventos.
